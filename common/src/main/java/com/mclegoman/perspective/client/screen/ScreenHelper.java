...
public class ScreenHelper {
	...
	public static GridWidget createTitle(MinecraftClient client, Screen parentScreen, boolean showPageName, @Nullable String pageName, boolean experimental, boolean updateMsg) {
		...
		if (Update.isNewerVersionFound() && updateMsg) {
			titleGridAdder.add(new TextWidget(Translation.getConfigTranslation(Data.getVersion().getID(), "update.title", new Formatting[]{Formatting.BOLD, Formatting.RED}), client.textRenderer));
			Text NEW_VERSION_TEXT = Translation.getConfigTranslation(Data.getVersion().getID(), "update.description", new Object[]{Update.latestVersionFound}, new Formatting[]{Formatting.YELLOW});
