...
public class ShadersConfigScreen extends AbstractConfigScreen {
	...
	private GridWidget createPageOne() {
		...
		try {
			...
			shaderOptionsGridAdder.add(ConfigButtonWidget.builder(() -> Translation.getConfigTranslation(Data.getVersion().getID(), "luminance"), (button) -> ClientData.minecraft.setScreen(new com.mclegoman.luminance.client.gui.screen.config.ConfigScreen(getRefreshScreen(), 0, SplashesDataloader.getSplashText(), PerspectiveLogo.isPride()))).width(304).build(), 2);
		} catch (Exception error) {
