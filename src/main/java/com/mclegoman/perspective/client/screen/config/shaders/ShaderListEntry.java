...
public class ShaderListEntry<E extends ShaderListEntry<E>> extends AlwaysSelectedEntryListWidget.Entry<E> {
	...
	public void render(DrawContext context, int index, int y, int x, int rowWidth, int rowHeight, int mouseX, int mouseY, boolean hovered, float delta) {
		context.drawCenteredTextWithShadow(ClientData.minecraft.textRenderer, Translation.getConfigTranslation(Data.getVersion().getID(), "shaders.super_secret_settings.list.shader", new Object[]{ShaderPacks.getRegistry().get(id).translation().getTranslation(ShaderPacks.shouldShowNamespace(ShaderPacks.getShadersId(), id))}), ClientData.minecraft.getWindow().getScaledWidth() / 2, y + (rowHeight / 2) - (9 / 2), 0xFFFFFF);
	}
	@Override
